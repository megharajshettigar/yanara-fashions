...
// ── VARIANT GROUPS ──
// Only TRUE same-design-different-colour products are grouped here.
// Grouped products show a colour switcher on the detail page.
// Everything NOT listed in a multi-code group stands alone (no switcher).
//
...
(function() {
  ...
  s.textContent =
    ...
    '.cswatch:hover,.cswatch.on{border-color:var(--gold);transform:scale(1.2)}' +
    '.color-opts{display:flex;gap:8px;margin-bottom:20px;flex-wrap:wrap;align-items:center}' +
    '.color-opt{display:flex;align-items:center;gap:6px;padding:6px 12px;border:1px solid var(--border);cursor:pointer;transition:all .2s;font-size:11px;letter-spacing:1px}' +
    '.color-opt:hover,.color-opt.on{border-color:var(--gold);color:var(--gold)}' +
    '.color-dot{width:14px;height:14px;border-radius:50%;flex-shrink:0;border:1px solid var(--swatch-border)}';
...
function injectColorPicker(activeId) { return;
  ...
  // variants, so we show no picker (and the old one is already removed above).
...
document.addEventListener('DOMContentLoaded', function() {
  setTimeout(function() {
    ...
    var _origCard = (function(fn){ return function(p,f){ return fn(p,f); }; })(window.card);
