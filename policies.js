...
// ── MODAL OPEN / CLOSE (used by all footer info modals) ──
// Defined here because the site footer links call these, but
// they weren't defined anywhere else.
...
// Inject the policy modals. For each policy, if a modal with
// that id already exists, update its CONTENTS (safer than
// swapping the whole node — keeps any existing event wiring).
...
(function injectPolicies(){
  function run(){
    ...
    newModals.forEach(function(m){
      ...
      if(existing){
        // keep the existing modal element (and its onclick),
        // just swap in the new inner content
        existing.innerHTML = m.innerHTML;
      } else {
