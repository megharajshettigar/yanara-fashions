...
const HOME_HTML = `
  <style>
    ...
    /* The star mask sits over the content and expands to reveal it */
    .ed-rev-inner{position:relative;z-index:2;opacity:0;transition:opacity .5s ease .35s}
    .ed-reviews.revealed .ed-rev-inner{opacity:1}
    /* The expanding 4-point star overlay (matches the reference video) */
    .ed-star-mask{position:absolute;inset:0;z-index:5;background:var(--bg2);pointer-events:none;
    ...
    .ed-reviews.revealed .ed-star-mask{animation:starWipe 1.15s cubic-bezier(.65,0,.35,1) forwards}
    /* the star GROWS from center outward, revealing content through it */
    @keyframes starWipe{
      0%{ -webkit-clip-path:polygon(50% 48%,52% 49%,52% 49%,51% 52%,50% 52%,49% 52%,48% 49%,48% 49%);
              clip-path:polygon(50% 48%,52% 49%,52% 49%,51% 52%,50% 52%,49% 52%,48% 49%,48% 49%) }
      100%{ -webkit-clip-path:polygon(50% -60%,75% 25%,160% 50%,75% 75%,50% 160%,25% 75%,-60% 50%,25% 25%);
                clip-path:polygon(50% -60%,75% 25%,160% 50%,75% 75%,50% 160%,25% 75%,-60% 50%,25% 25%) }
    }
...
// ── REVIEWS: star-reveal animation on scroll ──
function setupReviewReveal(){
  var section = document.getElementById("ed-reviews");
  if(!section) return;
  if(!("IntersectionObserver" in window)){ section.classList.add("revealed"); return; }
