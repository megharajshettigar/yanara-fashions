...
function setupReviewReveal(){
  ...
  function animateStar(){
    var dur = 1300;          // animation length (ms)
    var start = null;
    function frame(ts){
      if(!start) start = ts;
      ...
      // scale grows from ~0 to large enough to cover the whole section
      var scale = 0.02 + e * 2.2;
      path.setAttribute("d", buildStarPath(scale));
      if(t < 1){
        requestAnimationFrame(frame);
      } else {
        // fully revealed — remove the overlay so content is clean
