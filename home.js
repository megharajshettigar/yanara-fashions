...
const HOME_HTML = `
  <style>
    ...
    /* arched-top image (matches the hero arch) */
    .ed-pcard-img{position:relative;width:100%;height:360px;overflow:hidden;
      border-radius:140px 140px 18px 18px;background:var(--bg2)}
    ...
    .ed-pcard-badge{position:absolute;top:16px;left:16px;z-index:2;background:var(--gold);color:var(--bg);
      font-size:10px;letter-spacing:1.5px;text-transform:uppercase;padding:5px 12px;border-radius:20px;font-weight:600}
    ...
    @media (max-width:768px){
      /* Arched product cards: smaller, less stagger on mobile */
      .ed-pcard{width:200px}
      .ed-pcard-img{height:280px;border-radius:100px 100px 14px 14px}
...
// Build one arched staggered product card for the home rows.
function homeCard(p){
  ...
  return '<div class="ed-pcard" onclick="openP('+p.id+')">'+
    '<div class="ed-pcard-img">'+badge+img+
      '<button class="ed-pcard-wish" onclick="event.stopPropagation();toast(\'Saved to wishlist ♡\')"><i class="ti ti-heart"></i></button>'+
    '</div>'+
    '<div class="ed-pcard-info">'+
...
function renderHome(){
  ...
  if(na)na.innerHTML=products.slice(0,6).map(p=>homeCard(p)).join("");
  if(bs)bs.innerHTML=products.slice(4,10).map(p=>homeCard(p)).join("");
  setupCardReveal();
}
